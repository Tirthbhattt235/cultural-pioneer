...
export default function Search_result({title}) {
  ...
  useEffect(() => {
    ...
    const searchData = window.localStorage.getItem("Category-Items");
    setSearchResults(searchData || []);
  }, []);
  const [searchString, setSearchString] = useState();
  useEffect(() => {
    const  searchQuery  = title;
    console.log(title) // Get the search query from the router query object
        ...
        <Container>
          ...
          >
            <>
              ...
              <Breadcrumb SearchProp={` :${searchString}`} />
            </>
          </section>
        </Container>
        <SearchResult LocalProp={"Category-Items"}/>
        <Ftrswiper />
        <Footer></Footer>
      </>
      {/* ) : null} */}
